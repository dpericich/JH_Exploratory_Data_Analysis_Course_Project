## Plot 5 for Exploratory Data Analysis Course Project
...
balt_subset <- NEI[NEI$fips == "24510" & NEI$type == "ON-ROAD",]

total_by_year <- aggregate(Emissions ~ year, balt_subset, sum)
...
g <- g + geom_bar(stat="identity") + xlab("year") + ylab("Total PM2.5 Emissions") + 
  ggtitle("Total Emissions from motor vehicle sources between 1999 and 2008")
print(g)
dev.off()
